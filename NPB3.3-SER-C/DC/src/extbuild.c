...
int32 ComputeMemoryFittedView (ADC_VIEW_CNTL *avp){
  ...
  ib=0;
#ifdef USE_CITERATOR
  FOR_START(iRec, cit1, 1, avp->nRowsToRead+1, 1, cit_step_add, RND) {
  ...
  struct cit_data *cit1, *cit2;
...
#ifdef USE_CITERATOR
    FOR_START(i, cit1, 0, 2*avp->nm+avp->nv, 1, cit_step_add, RND) {
    /*for(i = 0; i < 2*avp->nm+avp->nv; i++){*/
      prevBuf[i] = 0; aggrBuf[i] = 0;
    }
    FOR_END(cit1);
#else
    ...
    /*for(iRec = 1; iRec <= avp->nRowsToRead; iRec++ ){*/
	  ...
	  case  1:
	    ...
	    nOut++;
#ifdef USE_CITERATOR
      FOR_START(i, cit2, 0, avp->nm, 1, cit_step_add, RND) {
			/*for ( i = 0; i < avp->nm; i++ ){*/
   ...
   nOut++;
#ifdef USE_CITERATOR
   FOR_START(i, cit1, 0, avp->nm, 1, cit_step_add, RND) {
   /*for ( i = 0; i < avp->nm; i++ ){*/
     avp->mSums[i] += aggrmp[i];
     avp->checksums[i] += nOut*aggrmp[i]%measbound;
   }
...
int32 MultiWayMerge(ADC_VIEW_CNTL *avp){
  ...
  struct cit_data *cit1;
...
#ifdef USE_CITERATOR
   FOR_START(i, cit1, 0, avp->numberOfChunks, 1, cit_step_add, RND) {
   ...
   while(1){
#ifdef USE_CITERATOR
     FOR_START(iChunk, cit1, 0, avp->numberOfChunks, 1, cit_step_add, RND) {
...
#ifdef USE_CITERATOR
     FOR_START(iChunk, cit1, min_r_chunk, avp->numberOfChunks, 1, cit_step_add, RND) {
...
#ifdef USE_CITERATOR
   j=0;
   FOR_START(i, cit1, 0, nv, 1, cit_step_add, FWD) {
   /*for ( j = 0, i = 0; i < nv; i++ ) {*/
...
#ifdef USE_CITERATOR
   FOR_START(i, cit1, 0, adccntl->nm, 1, cit_step_add, RND) {
...
#ifdef USE_CITERATOR
   FOR_START(j, cit1, 1, adcpp->nd, 1, cit_step_add, RND) {
...
#ifdef USE_CITERATOR
   FOR_START(i, cit1, 0, adccntl->nv, 1, cit_step_add, RND) {
