...
void adaptation(logical *ifmortar, int step)
{
  ...
  *ifmortar = false;
  ...
  //                morton index
#ifdef USE_CITERATOR
  FOR_START(miel, cit1, 0, nelt, 1, cit_step_add, RND) {
  ...
  /*for (miel = 0; miel < nelt; miel++) {*/
    ...
    if (ich[iel] == 2) {
      ...
      if (!btest(ic,0) && !btest(ic,1) && !btest(ic,2)) {
        ...
        test1 = true;
        FOR_START(i, cit2, 0, 8, 1, cit_step_add, RND) {
        /*for (i = 0; i < 8; i++) {*/
        ...
        if (test) {
          ...
          FOR_END(cit2);
          front[miel] = 1;
          FOR_START(i, cit2, 0, 7, 1, cit_step_add, RND) {
          /*for (i = 0; i < 7; i++) {*/
            skip[ntp[i+1]] = true;
          }
          FOR_END(cit2);
...
#ifdef USE_CITERATOR
  FOR_START(index, cit1, 0, num_coarsen, 1, cit_step_add, RND) {
...
static void do_refine(logical *ifmortar, int *irefine)
{
  ...
  /*for (index = 0; index < num_refine; index++) {*/
    ...
    FOR_START(j, cit2, 0, 7, 1, cit_step_add, RND) {
    /*for (j = 0; j < 7; j++) {*/
    ...
    FOR_START(j, cit2, 0, 7, 2, cit_step_add, RND) {
    /*for (j = 0; j < 7; j += 2) {*/
    ...
    FOR_START(i, cit2, 0, 7, 2, cit_step_add, RND) {
    /*for (i = 0; i < 7; i += 2) {*/
      xc[iel][i] = xleft;
      xc[iel][i+1] = xhalf;
    }
    ...
    FOR_START(i, cit2, 0, 2, 1, cit_step_add, RND) {
    /*for (i = 0; i < 2; i++) {*/
      yc[nelt+0][i] = yleft;
      yc[nelt+3][i] = yleft;
      yc[nelt+4][i] = yleft;
    ...
    FOR_END(cit2);
    FOR_START(i, cit2, 2, 4, 1, cit_step_add, RND) {
    /*for (i = 2; i < 4; i++) {*/
      yc[nelt+0][i] = yhalf;
      yc[nelt+3][i] = yhalf;
      yc[nelt+4][i] = yhalf;
    ...
    FOR_END(cit2);
    FOR_START(j, cit2, 1, 3, 1, cit_step_add, RND) {
    /*for (j = 1; j < 3; j++) {*/
      FOR_START(i, cit3, 0, 2, 1, cit_step_add, RND) {
      /*for (i = 0; i < 2; i++) {*/
        yc[nelt+j][i] = yhalf;
        yc[nelt+j+4][i] = yhalf;
        yc[nelt+j][i+4] = yhalf;
      ...
      FOR_END(cit3);
      FOR_START(i, cit3, 2, 4, 1, cit_step_add, RND) {
      /*for (i = 2; i < 4; i++) {*/
        yc[nelt+j][i] = yright;
    ...
    FOR_START(j, cit2, 0, 3, 1, cit_step_add, RND) {
    /*for (j = 0; j < 3; j++) {*/
      FOR_START(i, cit3, 0, 4, 1, cit_step_add, RND) {
      /*for (i = 0; i < 4; i++) {*/
        zc[nelt+j][i] = zleft;
        zc[nelt+j][i+4] = zhalf;
      }
    ...
    FOR_END(cit2);
    FOR_START(j, cit2, 3, 7, 1, cit_step_add, RND) {
    /*for (j = 3; j < 7; j++) {*/
      FOR_START(i, cit3, 0, 4, 1, cit_step_add, RND) {
      /*for (i = 0; i < 4; i++) {*/
        zc[nelt+j][i] = zhalf;
    ...
    FOR_END(cit2);
    FOR_START(i, cit2, 0, 4, 1, cit_step_add, RND) {
    /*for (i = 0; i < 4; i++) {*/
      zc[iel][i] = zleft;
      zc[iel][i+4] = zhalf;
    }
    ...
    FOR_START(ndir, cit2, 0, 3, 1, cit_step_add, RND) {
    /*for (ndir = 0; ndir < 3; ndir++) {*/
      FOR_START(facedir, cit3, 0, 2, 1, cit_step_add, RND) {
      /*for (facedir = 0; facedir <= 1; facedir++) {*/
        ...
        FOR_START(k, cit4, 0, 4, 1, cit_step_add, RND) {
        /*for (k = 0; k < 4; k++) {*/
...
#ifdef USE_CITERATOR
  FOR_START(iel, cit1, 0, neltold, 1, cit_step_add, RND) {
  /*for (iel = 0; iel < neltold; iel++) {*/
    if (!skip[iel]) {
      ich[iel] = 0;
      if (!iftouch(iel)) {
        iftemp = false;
        FOR_START(i, cit2, 0, NSIDES, 1, cit_step_add, RND) {
        /*for (i = 0; i < NSIDES; i++) {*/
    ...
    if (ich[iel] == 4) {
      ...
      /*for (i = 0; i < NSIDES; i++) {*/
        ...
        if (cbc[iel][i] == 1) {
          ...
          FOR_START(iface, cit3, 0, NSIDES, 1, cit_step_add, RND) {
                  ...
                  double x[LX1][LX1][LX1])
{
  ...
  /*for (i = 0; i < LX1; i++) {*/
    FOR_START(kk, cit2, 0, LX1, 1, cit_step_add, RND) {
    /*for (kk = 0; kk < LX1; kk++) {*/
      FOR_START(jj, cit3, 0, LX1, 1, cit_step_add, RND) {
      /*for (jj = 0; jj < LX1; jj++) {*/
        FOR_START(ii, cit4, 0, LX1, 1, cit_step_add, RND) {
        /*for (ii = 0; ii < LX1; ii++) {*/
...
static void merging(int iela[8])
{
  ...
  FOR_END(cit1);
  FOR_START(i, cit1, 1, 8, 2, cit_step_add, RND) {
  /*for (i = 1; i < 8; i += 2) {*/
    xc[ielnew][i] = x2;
  }
  FOR_END(cit1);
  ...
  FOR_END(cit1);
  FOR_START(i, cit1, 2, 4, 1, cit_step_add, RND) {
  /*for (i = 2; i < 4; i++) {*/
    yc[ielnew][i] = y2;
    yc[ielnew][i+4] = y2;
  }
  ...
  FOR_END(cit1);
  FOR_START(i, cit1, 4, 8, 1, cit_step_add, RND) {
  /*for (i = 4; i < 8; i++) {*/
    zc[ielnew][i] = z2;
  }
  FOR_END(cit1);
  ...
  // update neighboring information
#ifdef USE_CITERATOR
  FOR_START(i, cit1, 0, NSIDES, 1, cit_step_add, RND) {
  /*for (i = 0; i < NSIDES; i++) {*/
    jface = jjface[i];
    ielold = iela[children[i][0]];
    FOR_START(ii, cit2, 0, 4, 1, cit_step_add, RND) {
    /*for (ii = 0; ii < 4; ii++) {*/
...
#ifdef USE_CITERATOR
  FOR_START(iy, cit1, 0, LX1, 1, cit_step_add, RND) {
  /*for (iy = 0; iy < LX1; iy++) {*/
    FOR_START(ix, cit2, 0, LX1, 1, cit_step_add, RND) {
    /*for (ix = 0; ix < LX1; ix++) {*/
                   ...
                   double y[LX1][LX1][LX1])
{
  ...
  FOR_START(iz, cit1, 0, LX1, 1, cit_step_add, RND) {
  /*for (iz = 0; iz < LX1; iz++) {*/
    FOR_START(iy, cit2, 0, LX1, 1, cit_step_add, RND) {
    /*for (iy = 0; iy < LX1; iy++) {*/
