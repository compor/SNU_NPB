...
void CompExp(int n, dcomplex exponent[n])
{
  ...
  ln = 1;
#ifdef USE_CITERATOR
  FOR_START(j, cit1, 1, m+1, 1, cit_step_add, RND) {
  /*for (j = 1; j <= m; j++) {*/
    t = pi / ln;
    FOR_START(i, cit2, 0, ln - 1+1, 1, cit_step_add, RND) {
    /*for (i = 0; i <= ln - 1; i++) {*/
      ti = i * t;
...
#ifdef USE_CITERATOR
  FOR_START(i, cit1, 1, 1024+1, 1, cit_step_add, RND) {
  /*for (i = 1; i <= 1024; i++) {*/
    i1 = i;
    ii = i1 % d1;
    ji = 3 * i1 % d2;
                                ...
                                dcomplex u0[d3][d2][d1+1])
{
  ...
  RanStarts[0] = start;
#ifdef USE_CITERATOR
  FOR_START(k, cit1, 1, d3, 1, cit_step_add, FWD) {
  /*for (k = 1; k < d3; k++) {*/
    dummy = randlc(&start, an);
    RanStarts[k] = start;
  }
...
#ifdef USE_CITERATOR
  FOR_START(k, cit1, 0, d3-1+1, 1, cit_step_add, RND) {
  /*for (k = 0; k < d3; k++) {*/
    x0 = RanStarts[k];
    FOR_START(j, cit2, 0, d2, 1, cit_step_add, FWD) {
    /*for (j = 0; j < d2; j++) {*/
      vranlc(2*d1, &x0, a, (double *)tmp);
      FOR_START(i, cit3, 0, d1-1+1, 1, cit_step_add, RND) {
      /*for (i = 0; i < d1; i++) {*/
        u0[k][j][i] = tmp[i];
      }
      FOR_END(cit3);
...
#ifdef USE_CITERATOR
  FOR_START(i, cit1, 0, nz-1+1, 1, cit_step_add, RND) {
  /*for (i = 0; i < nz; i++) {*/
    FOR_START(k, cit2, 0, ny-1+1, 1, cit_step_add, RND) {
    /*for (k = 0; k < ny; k++) {*/
      FOR_START(j, cit3, 0, nx-1+1, 1, cit_step_add, RND) {
      /*for (j = 0; j < nx; j++) {*/
